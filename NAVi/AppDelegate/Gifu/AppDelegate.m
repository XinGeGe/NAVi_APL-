...
-(void)ClearTheoldfile{
    ...
    for (NSInteger index=0; index<myusersarray.count; index++) {
        NSString *lastdate=[[NASQLHelper sharedInstance]getThelastdateWithuserid:[myusersarray objectAtIndex:index]];
        [[NAFileManager sharedInstance]clearTheoldfileWithuserid:[myusersarray objectAtIndex:index] Thelastdate:lastdate];
    }
}
