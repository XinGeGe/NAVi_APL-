...
- (void)viewDidLoad {
    ...
    [clipBtn setTitle:@"お気に入り" forState:UIControlStateNormal];
    ...
    textSearch.placeholder = @"キーワードで記事を検索";
...
{
    NSString *strUrl = [textSearch.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    if (strUrl.length > 0) {
        NALog(@"searchBtnAction");
        [TAGManagerUtil pushButtonClickEvent:ENSerachBtn label:strUrl];
        NSString *result = [[NAFileManager sharedInstance] getSearchResult];
        if (result.length > 0) {
            if ([self.searchResults containsObject:strUrl]) {
                [self searchAPI:strUrl];
                NSMutableArray *arr = [[NSMutableArray alloc]initWithArray:[[[NAFileManager sharedInstance] getSearchResult] componentsSeparatedByString:@","]];
                NSInteger index = 0;
                for (int i = 0; i < arr.count; i++) {
                    if ([arr[i] isEqualToString:strUrl]) {
                        index = i;
                    }
                }
                [arr removeObjectAtIndex:index];
                
                [arr addObject:strUrl];
                ...
                for (int i = 0; i < arr.count; i++) {
                    if (i == 0) {
                        ...
                        result = [result stringByAppendingFormat:@",%@",arr[i]];
                    }
                }
            ...
            result = [result stringByAppendingFormat:@",%@",strUrl];
        }else {
            result = strUrl;
        }
...
- (void)searchAPI:(NSString *)keyword
{
    [textSearch resignFirstResponder];
    if (keyword.length > 0) {
        if (searchNumber == 0) {
            [self searchNoteAPI:keyword];
        }else{
            [self searchFavoriteAPI:keyword];
        }
    }
...
//    result.serachDate=@"mycurrDoc";
    ...
    result.isfromWhere = _isfromWhere;
...
- (void)searchNoteAPI:(NSString *)keyword
{
                               ...
                               completionBlock:^(id search, NSError *error) {
                                   ...
                                   if (!error) {
                                       dispatch_async(dispatch_get_main_queue(), ^{
                                           ...
                                           if (array.count > 0) {
                                               [self toSearchResultList:array KeyWord:keyword];
                                           }else{
                                               [ProgressHUD dismiss];
                                               [[[iToast makeText:NSLocalizedString(@"NO Note", nil)]
                                                 setGravity:iToastGravityBottom] show];
