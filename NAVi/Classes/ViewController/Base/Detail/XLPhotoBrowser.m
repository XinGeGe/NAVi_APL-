...
- (UIButton *)btnCancel{
    if (!_btnCancel) {
        ...
        _btnCancel.frame = CGRectMake(10, 20, 25, 25);
        [_btnCancel setImage:[UIImage imageNamed:@"20_white"] forState:UIControlStateNormal];
...
- (void)setUpToolBars {
    ...
    CGFloat f = [[self class] heightWithText:[_headtitleArr objectAtIndex:0]];
    indexLabel.bounds = CGRectMake(0, 0, self.xl_width, f+10);
    indexLabel.xl_centerX = self.xl_width * 0.5;
    indexLabel.xl_centerY = self.xl_height-f-10;
    indexLabel.textAlignment = NSTextAlignmentLeft;
    indexLabel.textColor = [UIColor whiteColor];
    indexLabel.font = [UIFont systemFontOfSize:15];
    indexLabel.backgroundColor = [UIColor colorWithRed:118.0/255.0 green:118.0/255.0 blue:118.0/255.0 alpha:126.0/255.0];
    indexLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    indexLabel.numberOfLines = 0;
    self.indexLabel = indexLabel;
    self.indexLabel.text = [_headtitleArr objectAtIndex:0];
...
- (void)saveImage {
    ...
    if (zoomingScrollView.progress < 1.0) {
        self.savaImageTipLabel.text = NSLocalizedString(XLPhotoBrowserLoadingImageText, nil);
...
- (void)updatePageControlIndex {
    ...
    self.indexLabel.text = [_headtitleArr objectAtIndex:self.currentImageIndex];
 ...
 */
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex title:(NSArray *)title{
...
//    }
    ...
    browser.headtitle = [title objectAtIndex:0];
    [browser show];
    return browser;
}
