...
@interface SwipeView () <UIScrollViewDelegate, UIGestureRecognizerDelegate,NALoginAlertViewDelegate,NALoginReminAlertViewDelegate>
{
...
- (void)setUp
{
    ...
    [_scrollView addGestureRecognizer:self.leftSwipeGestureRecognizer];
    [_scrollView addGestureRecognizer:self.rightSwipeGestureRecognizer];
...
- (void)setItemView:(UIView *)view forIndex:(NSInteger)index
{
    ...
    if ([_delegate respondsToSelector:@selector(getItemView:forIndex:)])
    {
        [_delegate getItemView:view forIndex:index];
    }
}
