...
- (instancetype)init {
    self = [super init];
    if (self) {
        _tagTextFont = [UIFont systemFontOfSize:15.0f];
...
- (void)commonInit {
    ...
    _tagCollectionView = [[TTGTagCollectionView alloc] initWithFrame:CGRectZero];
...
- (void)insertTags:(NSArray<NSString *> *)tags atIndex:(NSUInteger)index withConfig:(TTGTextTagConfig *)config copyConfig:(BOOL)copyConfig {
    ...
    for (NAClipDoc *tagText in tags) {
        TTGTextTagLabel *label = [self newLabelForTagText:[tagText.tagName description] withConfig:config];
        label.tagId = tagText.tagid;
...
- (NSArray <NSString *> *)allSelectedTags {
    ...
    for (TTGTextTagLabel *label in _tagLabels) {
        if (label.selected) {
            NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:label.label.text,label.tagId, nil];
            [allTags addObject:dic];
        }
    }
